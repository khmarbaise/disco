...
+----------------+-----------------+--------------------+
```
You can limit this by using the `--version Verison` option with
...
+------+---------------+--------------------------------+
```
You can enhance the above call with the supplemental option `--verbose`
...
+----------------+-----------------+--------------------------------+
```
Furthermore you can use the `major_versions` end-point which looks like this:
````
> ./disco mv 
